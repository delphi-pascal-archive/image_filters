unit untFiltreDivImageConstante;
...
type
  TfrmFiltreDivImageConstante = class(TfrmHFiltreImageConstante)
...
var
  frmFiltreDivImageConstante: TfrmFiltreDivImageConstante;
...
{ TfrmHFiltreImageConstante4 }

procedure TfrmFiltreDivImageConstante.PrevisualiseResultat;
var
  X, Y : Integer;
  X2, Y2 : Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      if Couleur.Rouge = 0 then
        ImageResultat.Image[X, Y].Rouge := Image.Image[X, Y].Rouge
      else
        ImageResultat.Image[X, Y].Rouge := Image.Image[X, Y].Rouge / Couleur.Rouge;  // Divise

      if Couleur.Vert = 0 then
        ImageResultat.Image[X, Y].Vert := Image.Image[X, Y].Vert
      else
        ImageResultat.Image[X, Y].Vert := Image.Image[X, Y].Vert / Couleur.Vert;  // les pixels de l'image

      if Couleur.Bleu = 0 then
        ImageResultat.Image[X, Y].Bleu := Image.Image[X, Y].Bleu
      else
...
procedure TfrmFiltreDivImageConstante.btnOKClick(Sender: TObject);
begin
  inherited;
