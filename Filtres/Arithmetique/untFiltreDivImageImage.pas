...
type
  TfrmFiltreDivImageImage = class(TfrmHFiltreImageImage)
...
var
  frmFiltreDivImageImage: TfrmFiltreDivImageImage;
...
{ TfrmDivFiltreImageImage }

procedure TfrmFiltreDivImageImage.PrevisualiseResultat;
var
  X, Y : Integer;
  X2, Y2 : Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      if Image2.Image[X, Y].Rouge = 0 then
        ImageResultat.Image[X, Y].Rouge := Image1.Image[X, Y].Rouge
      else
        ImageResultat.Image[X, Y].Rouge := Image1.Image[X, Y].Rouge / Image2.Image[X, Y].Rouge;  // Divise

      if Image2.Image[X, Y].Vert = 0 then
        ImageResultat.Image[X, Y].Vert := Image1.Image[X, Y].Vert
      else
        ImageResultat.Image[X, Y].Vert := Image1.Image[X, Y].Vert / Image2.Image[X, Y].Vert;  // les pixels de l'image 1

      if Image2.Image[X, Y].Bleu = 0 then
        ImageResultat.Image[X, Y].Bleu := Image1.Image[X, Y].Bleu
      else
...
procedure TfrmFiltreDivImageImage.btnOKClick(Sender: TObject);
begin
  inherited;
