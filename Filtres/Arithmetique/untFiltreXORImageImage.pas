...
type
  TfrmFiltreXORImageImage = class(TfrmHFiltreImageImage)
...
var
  frmFiltreXORImageImage: TfrmFiltreXORImageImage;
...
{ TfrmHFiltreImageImage1 }

procedure TfrmFiltreXORImageImage.PrevisualiseResultat;
var
  X, Y : Integer;
  X2, Y2 : Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      ImageResultat.Image[X, Y].Rouge := LimiteCouleur(Image1.Image[X, Y].Rouge) xor LimiteCouleur(Image2.Image[X, Y].Rouge);  // Applique l'opérateur XOR entre
      ImageResultat.Image[X, Y].Vert := LimiteCouleur(Image1.Image[X, Y].Vert) xor LimiteCouleur(Image2.Image[X, Y].Vert);  // les pixels de l'image 1
      ImageResultat.Image[X, Y].Bleu := LimiteCouleur(Image1.Image[X, Y].Bleu) xor LimiteCouleur(Image2.Image[X, Y].Bleu);  // et les pixels de l'image 2
    end;
  end;
  frmPrincipale.FinCalcul;
end;

procedure TfrmFiltreXORImageImage.btnOKClick(Sender: TObject);
begin
  inherited;
  TfrmMDIImage(ListeImages.Last).Caption := Format('(%s) XOR (%s)', [TfrmMDIImage(Image1).Caption, TfrmMDIImage(Image2).Caption]);  // Affiche le titre de l'image
