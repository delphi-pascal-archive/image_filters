...
type
  TfrmFiltreANDImageImage = class(TfrmHFiltreImageImage)
...
procedure TfrmFiltreANDImageImage.PrevisualiseResultat;
var
  X, Y : Integer;
  X2, Y2 : Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      ImageResultat.Image[X, Y].Rouge := LimiteCouleur(Image1.Image[X, Y].Rouge) and LimiteCouleur(Image2.Image[X, Y].Rouge);  // Applique l'opérateur AND entre
      ImageResultat.Image[X, Y].Vert := LimiteCouleur(Image1.Image[X, Y].Vert) and LimiteCouleur(Image2.Image[X, Y].Vert);  // les pixels de l'image 1
      ImageResultat.Image[X, Y].Bleu := LimiteCouleur(Image1.Image[X, Y].Bleu) and LimiteCouleur(Image2.Image[X, Y].Bleu);  // et les pixels de l'image 2
    end;
  end;
...
begin
  inherited;
  TfrmMDIImage(ListeImages.Last).Caption := Format('(%s) AND (%s)', [TfrmMDIImage(Image1).Caption, TfrmMDIImage(Image2).Caption]);  // Affiche le titre de l'image
