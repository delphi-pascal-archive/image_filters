unit untFiltreORImageConstante;
...
type
  TfrmFiltreORImageConstante = class(TfrmHFiltreImageConstante)
...
var
  frmFiltreORImageConstante: TfrmFiltreORImageConstante;
...
{ TfrmFiltreORImageConstante }

procedure TfrmFiltreORImageConstante.PrevisualiseResultat;
var
  X, Y : Integer;
  X2, Y2 : Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      ImageResultat.Image[X, Y].Rouge := LimiteCouleur(Image.Image[X, Y].Rouge) or LimiteCouleur(Couleur.Rouge);  // Applique l'opérateur OR entre
      ImageResultat.Image[X, Y].Vert := LimiteCouleur(Image.Image[X, Y].Vert) or LimiteCouleur(Couleur.Vert);  // les pixels de l'image
      ImageResultat.Image[X, Y].Bleu := LimiteCouleur(Image.Image[X, Y].Bleu) or LimiteCouleur(Couleur.Bleu);  // et la couleur
    end;
  end;
  frmPrincipale.FinCalcul;
end;

procedure TfrmFiltreORImageConstante.btnOKClick(Sender: TObject);
begin
  inherited;
  TfrmMDIImage(ListeImages.Last).Caption := Format('(%s) OR RGB(%g, %g, %g)', [TfrmMDIImage(Image).Caption, Couleur.Rouge, Couleur.Vert, Couleur.Bleu]);  // Affiche le titre de l'image
