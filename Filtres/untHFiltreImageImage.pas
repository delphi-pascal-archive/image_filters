...
begin
  ...
  begin
    ...
    lstImage2.Items.Add(TfrmMDIImage(ListeImages.Items[NumImage]).Caption);  // images à la liste
  end;
end;
...
begin
  ...
  Bitmap.Canvas.Brush := lstImage1.Canvas.Brush;  // mêmes propriétés
...
begin
  ...
  Bitmap.Canvas.Brush := lstImage2.Canvas.Brush;  // mêmes propriétés
...
begin
  if (lstImage1.ItemIndex > -1) and (lstImage2.ItemIndex > -1) then  // Si 2 images sont sélectionnée
    Result := (TfrmMDIImage(Image1).CalcImage.TailleX = TfrmMDIImage(Image2).CalcImage.TailleX) and (TfrmMDIImage(Image1).CalcImage.TailleY = TfrmMDIImage(Image2).CalcImage.TailleY)  // Vérifie si les 2 images sélectionnées sont de la même taille
  else
    Result := False;
end;
...
begin
  Image1 := ListeImages.Items[lstImage1.ItemIndex];  // Pointe sur l'image sélectionnée

  imgPrevisualisation.Canvas.Rectangle(0, 0, imgPrevisualisation.Width, imgPrevisualisation.Height);  // Efface l'image de la prévisualisation
  btnOK.Enabled := EnabledOK;  // Vérifie si les images sont "compatibles"
  PrevisualiseResultat;
end;

procedure TfrmHFiltreImageImage.lstImage2Click(Sender: TObject);
begin
  Image2 := ListeImages.Items[lstImage2.ItemIndex];  // Pointe sur l'image sélectionnée
